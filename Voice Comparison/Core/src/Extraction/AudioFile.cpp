...
{
	if (size&& audio) {
		//unsigned int total_length = (size*(unsigned int)channels);
		//for (unsigned int i = 0; total_length > i; i++){
			//delete audio;
		//}
		delete [] audio;
		audio = NULL;
...
{
	// open file
	if (filename[0] == 0) return NO_FILE;
	if ((file = fopen(filename,"rb")) == NULL) return NO_FILE;
	
	// check file type
	int err;
	unsigned int tag;
	fread (&tag, sizeof(int), 1, file); // Load type file  --> first 4 byte contant type of wav file RIFF or FORM
	if (tag == T_RIFF) {
		err = LoadWave(offset, frames, nodata);
	}
	else if (tag == T_FORM) {
		err = LoadAiff(offset, frames, nodata);
	}
	else {err = FMTERR;} 
...
int AudioFile::LoadWave(unsigned int offset, unsigned int frames, bool nodata)
{
	...
	// check subfile type  
	fseek (file,4,SEEK_CUR); // Pass Cursor to cksize
	fread (&data32, sizeof(int), 1, file); // Load  WAVEID
	...
	if (pformat == F_EXT) {			// 0xFFFE	WAVE_FORMAT_EXTENSIBLE	Determined by SubFormat					// read subformat tag if 
	...
	///bytesperword = Number of byte on one sample
	//blockalign  == NumChannels * BitsPerSample/8  Byte of 1 sample on all channel
	...
	fread (&chunksize, sizeof(int), 1, file);
	//chunksize = Subchunk2Size    == NumSamples * NumChannels * BitsPerSample/8
	psize = (chunksize/bytesperword)/pchannels;// chunksize /()
	...
	nofsamples = psize*pchannels; // all data both channel and sample
...
int AudioFile::SaveWave(char *filename)
{
	...
	// open file
	if (filename[0] == 0) return NO_FILE;
	if ((file = fopen(filename,"wb")) == NULL) return NO_FILE;
...
int AudioFile::AppendWave(char *filename)
{
	...
	if ((channels == 0) || (rate == 0)) return NOSUPPORT;
	if (filename[0] == 0) return NO_FILE;
	if ((file = fopen(filename,"r+b")) == NULL) return NO_FILE;
	...
	if (size > (((UINT_MAX-oldallcsize-1)/bytesperword)/channels)) 
		{fclose(file); return UNSAVED;}					// chunk size becomes too l
