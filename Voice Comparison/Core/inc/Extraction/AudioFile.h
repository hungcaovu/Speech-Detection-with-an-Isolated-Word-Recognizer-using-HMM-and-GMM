...
const int NO_FILE	= -1;				// file not found
...
private:								//*** see AudioFile.cpp for details	***	
	...
	float getmaxabs();
	int LoadWave(unsigned int offset, unsigned int frames, bool nodata);	 // Load RIFF	
	int LoadAiff(unsigned int offset, unsigned int frames, bool nodata);	// Load FORM			
	double ReadExtFloat();
