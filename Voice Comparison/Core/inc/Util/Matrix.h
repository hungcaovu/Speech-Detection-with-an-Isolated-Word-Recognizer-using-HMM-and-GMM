...
{
  friend class Vector;
  friend Matrix operator /(DATA_TYPE val, const Matrix& in);
  bool diag;
protected: 
...
public:
  ...
  inline Matrix(const DATA_TYPE _[], unsigned int rowSize, unsigned int colSize)
  {
	diag = false;
    row       = 0;
    column    = 0;
  ...
  inline Matrix& ZeroColumn(unsigned int col)
  {
	  for (unsigned int j = 0; j < row; j++)
			  _[j*column + col] = 0.0f;
	  return *this;
  ...
  inline Matrix& ZeroRow(unsigned int row)
  {
  ...
  inline DATA_TYPE *Array() const{
    return _;
  }

  inline DATA_TYPE& operator() (const unsigned int row, const unsigned int col)
  {
	  return GetData(row, col);
  }

  inline DATA_TYPE& GetData(const unsigned int row, const unsigned int col){
  ...
  inline Matrix& operator += (DATA_TYPE scalar)
  {
    for (unsigned int j = 0; j < row; j++)
      for (unsigned int i = 0; i < column; i++)
        _[j*column+i] += scalar;
  ...
  inline Matrix operator + (DATA_TYPE scalar) const
  {
    Matrix result(row,column,false);  
    return Add(scalar,result);    
  }

  inline Matrix& Add(DATA_TYPE scalar, Matrix& result) const
  {
    result.Resize(row,column,false);
    for (unsigned int j = 0; j < row; j++)
      for (unsigned int i = 0; i < column; i++)
  ...
  inline Matrix& Sub(DATA_TYPE scalar, Matrix& result) const
  {
    result.Resize(row,column,false);
    for (unsigned int j = 0; j < row; j++)
      for (unsigned int i = 0; i < column; i++)
  ...
  inline Matrix& Mult(DATA_TYPE scalar, Matrix& result) const
  {
    result.Resize(row,column,false);
    for (unsigned int j = 0; j < row; j++)
      for (unsigned int i = 0; i < column; i++)
  ...
  inline Matrix& Div(DATA_TYPE scalar, Matrix& result) const
  {
    result.Resize(row,column,false);
    for (unsigned int j = 0; j < row; j++)
      for (unsigned int i = 0; i < column; i++)
  ...
  // Nhan = Matrix (M x N) dot Matrix (N x 1) = Matrix (M x 1)
  ...
  inline Matrix& Random(){
    for (unsigned int j = 0; j < row; j++)
      for (unsigned int i = 0; i < column; i++)
		  _[j*column + i] = ((DATA_TYPE)rand()) / ((DATA_TYPE)(RAND_MAX + 1.0));
    return *this;    
  }
  ...
  inline Matrix Inverse(DATA_TYPE *determinant = NULL) const
  {
    Matrix result;
    return Inverse(result,determinant);
  }  

  inline Matrix& Inverse(Matrix &result, DATA_TYPE *determinant = NULL) const
  {
    bInverseOk = true;       
    if(row==column){ // Square matrix
		if (determinant != NULL) *determinant = DATA_TYPE(1.0);
      result.Resize(row,column,false);
      const unsigned int n = row;
      Matrix MM(*this);
      result.Identity();
      for(unsigned int i=0;i<n;i++){
		  DATA_TYPE pivot = MM._[i*column + i];
        ...
        for(unsigned int k=0;k<n;k++){
          if(k!=i){
			  const DATA_TYPE mki = MM._[k*column + i];
            for(unsigned int j=0;j<n;j++){
  ...
  inline Vector Mean(bool useRowAsDim = true){
	  COUNT_TYPE size = (useRowAsDim) ? column : row;
	  COUNT_TYPE dim = (useRowAsDim) ? row : column;

	  Vector result(dim);
	  for (COUNT_TYPE i = 0; i < size; i++){
		  for (COUNT_TYPE j = 0; j < dim; j++){
  ...
  void Standardization(bool useCol = true){
	  if (useCol){
		  Vector mean;
		  mean = SumColumn();
		  mean /= (DATA_TYPE)column;
  ...
  Vector Var(Vector &mean, bool isDimRow = true){
	  Vector result(mean.Size());
	  COUNT_TYPE size = isDimRow ? column : row;
	  COUNT_TYPE dim = isDimRow ? row : column;
	  for (COUNT_TYPE i = 0; i < size; i++){ // Data size
		  for (COUNT_TYPE j = 0; j < dim; j++){ // Dim

			  result[j] += pow((GetData(j, i)- mean[j]), 2);
		  }
	  }
	  result /= DATA_TYPE(size);
  ...
  Matrix RemoveColumn(COUNT_TYPE idx){
	  int cc = 0;
	  Matrix result(row, column - 1);
	  for (COUNT_TYPE c = 0; c < column; c++){
		  if (c == idx) continue;
		  int rr = 0;
		  for (COUNT_TYPE r = 0; r < row; r++){
  ...
  Matrix RemoveRow(COUNT_TYPE idx){
	  int cc = 0;
	  Matrix result(row - 1, column);
	  for (COUNT_TYPE c = 0; c < column; c++){
		  int rr = 0;
		  for (COUNT_TYPE r = 0; r < row; r++){
			  if (r == idx) continue;
  ...
  Matrix& ClearRow(unsigned int row){
	  if (row<this->row){
  ...
  Matrix RevertRowToCol(){
	  Matrix result;
	  return RevertRowToCol(result);
  }

  Matrix &RevertRowToCol( Matrix & result){
	  
	  result.Resize(column, row, false);

	  for (COUNT_TYPE i = 0; i < row; i++){
		  for (COUNT_TYPE j = 0; j < column; j++){
			  result.GetData(j, i) = GetData(i, j);
		  }
	  }
	  return result;
  }

  Matrix SubMatrixCol(COUNT_TYPE colFromIdx, COUNT_TYPE colToIdx = 0){
	  COUNT_TYPE resultColSize = 0;
	  if (colToIdx == 0) colToIdx = column -1;
	  Matrix result;
	  if (colToIdx > colFromIdx && colToIdx <=column){
		  resultColSize = colToIdx - colFromIdx;
		  result.Resize(row, resultColSize + 1);
		  for (COUNT_TYPE col = 0; col <= resultColSize; col++){
			  for (COUNT_TYPE r = 0; r < row; r++){
				  result.GetData(r, col) = GetData(r, col + colFromIdx);
			  }
		  }
	  }
	  return result;
  ...
  bool AppendColumn(Matrix &in){
	  COUNT_TYPE colSize = in.ColumnSize();
	  COUNT_TYPE rowSize = in.RowSize();
	  COUNT_TYPE oldCol = column;
	  if (rowSize == row || row == 0){
		  this->Resize(rowSize, column + colSize);
		  for (COUNT_TYPE col = 0; col < colSize; col++){
			  for (COUNT_TYPE r = 0; r < row; r++){
				  this->GetData(r, oldCol + col) = in(r, col);
			  }
		  }
		  return true;
	  }
  ...
  DATA_TYPE Max(){
	  DATA_TYPE max = GetData(0, 0);
	  for (COUNT_TYPE r = 0; r < row; r++){
		  for (COUNT_TYPE c = 0; c < column; c++){
			  DATA_TYPE data = GetData(r, c);
			  if (max < data){
				  max = data;
			  }
		  }
...
public:  
  inline virtual void Resize(unsigned int rowSize, unsigned int colSize, bool copy = true){
    if((row!=rowSize)||(column!=colSize)){
      if((rowSize)&&(colSize)){
		  DATA_TYPE *arr = ArrayUtil::CreateZeroArray1D(rowSize*colSize);//new float[rowSize*colSize];
        if(copy){
...
inline Matrix log(Matrix &m){
	COUNT_TYPE row = m.RowSize();
	COUNT_TYPE col = m.ColumnSize();
...
inline Matrix operator /(DATA_TYPE val, const Matrix& in)
{
	Matrix result(in);
	
	for (COUNT_TYPE row = 0; row < in.row; row++){
		for (COUNT_TYPE col = 0; col < in.column; col++){
			if (abs(in._[row*in.column + col]) > EPSILON){
				result._[row*in.column + col] = val / in._[row*in.column + col];
			}
			else {
				result._[row*in.column + col] = INIT_DATA_TYPE;
			}
		}
	}
	return result;
